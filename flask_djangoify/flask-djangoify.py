...
from flask_djangoify.utils import set_if_exists, get_config_processor, JSONEncoder
...
class Djangoify(object):
    ...
    def init_app(self, app, *args, **kwargs):
        ...
        app.__init__(*args, static_folder=None, **kwargs)
    ...
    def load_apps():
        for bp_mod in current_app.config.get('APPS', []):
            current_app.register_blueprint(bp_mod)

    @staticmethod
    def finalize_create():
        # connect nav list to contextprosessor
        current_app.configure()['apps'] = frozenset([x.name for x in current_app.blueprints.values()])
        current_app.configure(get_config_processor(current_app))
